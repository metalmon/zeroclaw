...
async fn run_heartbeat_worker(
    ...
    loop {
        ...
        // worker used to do locally, and — because it is the SAME pool
        // every other scoped `agent::run` call borrows from — this also
        // means the heartbeat reuses the exact pooled connection instead
        // of spawning/maintaining a second one.
