...
cli-doctor-config-file-missing = config file not found: {$path}
cli-doctor-provider-invalid = model_provider "{$label}" is invalid: {$reason}
cli-doctor-provider-valid = model_provider "{$label}" is valid
cli-doctor-api-key-configured = {$label}: API key configured
cli-doctor-provider-api-key-missing = {$label}: no api_key set (may rely on env vars or model_provider defaults)
cli-doctor-provider-model = {$label}: model: {$model}
cli-doctor-provider-model-missing = {$label}: no model configured
cli-doctor-no-model-providers = no model providers configured
cli-doctor-provider-temperature-ok = {$label}: temperature {$temperature} (valid range 0.0–2.0)
cli-doctor-provider-temperature-out-of-range = {$label}: temperature {$temperature} is out of range (expected 0.0–2.0)
cli-doctor-provider-temperature-unset = {$label}: temperature unset (provider default)
cli-doctor-provider-api-key-not-registered = {$label}: no api_key set — this provider will NOT register (not a soft fallback); set `[{$label}].api_key` or remove the entry
cli-doctor-gateway-port = gateway port: {$port}
cli-doctor-gateway-port-invalid = gateway port is 0 (invalid)
cli-doctor-model-route-empty-hint = model route with empty hint
cli-doctor-model-route-invalid-provider = model route "{$hint}" uses invalid model_provider "{$provider}": {$reason}
cli-doctor-model-route-empty-model = model route "{$hint}" has empty model
cli-doctor-embedding-route-empty-hint = embedding route with empty hint
cli-doctor-embedding-route-invalid-provider = embedding route "{$hint}" uses invalid model_provider "{$provider}": {$reason}
cli-doctor-embedding-route-empty-model = embedding route "{$hint}" has empty model
cli-doctor-embedding-route-invalid-dimensions = embedding route "{$hint}" has invalid dimensions=0
cli-doctor-embedding-hint-no-route = memory.embedding_model uses hint "{$hint}" but no matching [[embedding_routes]] entry exists
cli-doctor-channel-present = at least one channel configured
cli-doctor-no-channels = no channels configured — run `zeroclaw quickstart` to set one up
cli-doctor-telegram-bot-token-unset = channels.telegram.{$alias}.bot_token is unset but the channel is enabled — the channel cannot connect until a bot token is set
cli-doctor-discord-bot-token-unset = channels.discord.{$alias}.bot_token is unset but the channel is enabled — the channel cannot connect until a bot token is set
cli-doctor-agent-invalid-provider = agent "{$name}" uses invalid model_provider "{$provider}": {$reason}
cli-doctor-config-warning = {$message} (at {$path})
cli-doctor-workspace-exists = directory exists: {$path}
cli-doctor-workspace-missing = directory missing: {$path}
cli-doctor-workspace-writable = directory is writable
cli-doctor-workspace-write-probe-failed = directory write probe failed: {$error}
cli-doctor-workspace-not-writable = directory is not writable: {$error}
cli-doctor-disk-space-ok = disk space: {$available} MB available
cli-doctor-disk-space-low = low disk space: only {$available} MB available
cli-doctor-agent-file-present = [{$alias}] {$name} present
cli-doctor-agent-file-missing = [{$alias}] {$name} not found (optional)
cli-doctor-daemon-state-missing = state file not found: {$path} — is the daemon running?
cli-doctor-daemon-state-read-failed = cannot read state file: {$error}
cli-doctor-daemon-state-invalid-json = invalid state JSON: {$error}
cli-doctor-daemon-heartbeat-fresh = heartbeat fresh ({$age}s ago)
cli-doctor-daemon-heartbeat-stale = heartbeat stale ({$age}s ago)
cli-doctor-daemon-timestamp-invalid = invalid daemon timestamp: {$timestamp}
cli-doctor-scheduler-healthy = scheduler healthy (last ok {$age}s ago)
cli-doctor-scheduler-unhealthy = scheduler unhealthy (ok={$ok}, age={$age}s)
cli-doctor-scheduler-not-tracked = scheduler component not tracked yet
cli-doctor-channel-fresh = {$name} fresh ({$age}s ago)
cli-doctor-channel-stale = {$name} stale (ok={$ok}, age={$age}s)
cli-doctor-no-channel-components = no channel components tracked yet
cli-doctor-channels-stale-summary = {$count} channels, {$stale} stale
cli-doctor-shell-set = shell: {$shell}
cli-doctor-shell-unset = neither $SHELL nor %ComSpec% is set
cli-doctor-home-set = home directory env set
cli-doctor-home-unset = neither $HOME nor $USERPROFILE is set
cli-doctor-no-cli-tools = No CLI tools found in PATH
cli-doctor-cli-tool-entry = {$name} ({$category}) — {$version}
cli-doctor-cli-tools-count = {$count} CLI tools discovered
cli-doctor-command-version = {$cmd}: {$version}
cli-doctor-command-nonzero = {$cmd} found but returned non-zero
cli-doctor-command-not-found = {$cmd} not found in PATH
