...
/// Per-file decoded size limit for ACP embedded blobs (matches RPC attach).
...
/// Error while decoding or persisting an ACP embedded blob.
#[derive(Debug)]
...
mod tests {
    ...
    fn writes_blob_under_uploads_and_returns_document_marker() {
        ...
        assert!(out.marker.contains(out.abs_path.to_string_lossy().as_ref())
            || out
                .marker
                .contains(strip_windows_verbatim_prefix(&out.abs_path.to_string_lossy()).as_ref()));
