    ...
    pub fn parse_webhook_payload(&self, payload: &serde_json::Value) -> Vec<ChannelMessage> {
        ...
        if !self.is_sender_allowed(&normalized_from) {
            ::zeroclaw_log::record!(
                ...
                "ignoring message from unauthorized sender: . Add to channels.linq.allowed_senders in config.toml, or run `voltd config set channels.linq.allowed-senders='[\"<sender-id>\"]'`."
            );
            return messages;
        }
