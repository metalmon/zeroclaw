...
/// Public-facing router: exposes only the ACP WebSocket bridge (`/acp`).
/// This is the surface intended for a listener that is reachable beyond
...
"#,
    ...
    /// `public_router` must serve `/acp` and must NOT serve anything else —
    /// this is the whole point of the public/private split (the public
    /// listener is the one surface that may end up reachable beyond
    /// loopback, so it must carry nothing but the self-authenticating ACP
    /// bridge).
    #[tokio::test]
    async fn public_router_serves_only_acp() {
        ...
        assert_eq!(
            ...
            StatusCode::NOT_FOUND,
            "public_router must not serve private control-plane routes like /api/status"
        );
    }

    /// `private_router` is the mirror image: it must carry every
    /// control-plane route (e.g. `/api/status`, `/pair`) and must NOT
    /// carry `/acp` — that route belongs solely to `public_router`.
    #[tokio::test]
    async fn private_router_serves_everything_except_acp() {
        ...
        assert_ne!(
            ...
            StatusCode::NOT_FOUND,
            "private_router must serve /api/status"
        );
