    ...
    let paths = serde_json::json!({
        ...
        "/api/config": {
            "patch": {
                ...
                "description": "Operations execute in order against an in-memory copy; `Config::validate()` runs once at the end; on success the snapshot persists and swaps. On failure, on-disk and in-memory state are unchanged. `move`/`copy` return `op_not_supported`. `test` against a secret path returns `secret_test_forbidden`.\n\n**Drift guard:** if the on-disk file has drifted from in-memory state on any path being patched, returns 409 `config_changed_externally` unless the request carries `X-Volt-Override-Drift: true` (the legacy `X-ZeroClaw-Override-Drift` name is still accepted). GET /api/config/drift to inspect first.",
                "parameters": [{
                    "name": "X-Volt-Override-Drift",
                    "in": "header",
                    "required": false,
