...
- `tool_result_image_policy`: handling for image markers in native `role = "tool"` results sent to compatible chat-completions providers. Defaults to `"relocate"`: the image is kept but moved into a following `role = "user"` message, so endpoints that reject image parts in a tool result (many OpenAI-compatible endpoints return HTTP 400) still receive the image without any per-provider config. Set to `"image_url"` to keep structured image parts inside the tool message (for endpoints that accept them), or to `"omit"` to remove image URI/base64 payloads and append a fixed notice. This does not change direct user images or OpenAI Responses providers.
