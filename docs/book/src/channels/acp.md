...
When running `zeroclaw acp` as a subprocess, the command starts the server unconditionally. When running as a daemon, the gateway exposes ACP over WebSocket at `/acp` with no additional config required. Gateway clients may append `?agent=<alias>` to that URL so each configured agent can be addressed from a spec-vanilla one-agent-per-endpoint client; the query parameter grants no access beyond selecting among already-configured agents.

Authentication (`Authorization`, `Sec-WebSocket-Protocol`, or `?token=`) is checked before the socket is upgraded, but a missing or not-yet-paired token is not a hard rejection: when pairing is required, the connection is still upgraded in a **pre-auth** state that accepts only the `zeroclaw/pair` request. A successful pair resolves a principal on that same connection and every other method becomes available; an idle pre-auth connection that never pairs is dropped after 30 seconds. A presented token that resolves to `Denied` is still rejected with a hard `401` at upgrade time. This lets a network client reach `/acp` and pair in-band without a separate REST round trip. See [Operations → Network deployment](../ops/network-deployment.md#publicprivate-surface-split-gatewaypublic) for how this interacts with the `[gateway.public]` surface split, where `/acp` is the only route exposed on the public listener and REST `/pair` stays on the private surface.
