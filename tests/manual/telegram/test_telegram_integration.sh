...
if [ -f "target/release/voltd" ]; then
    BINARY_SIZE=$(ls -lh target/release/voltd | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/voltd 2>/dev/null || stat -c%s target/release/voltd)
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/voltd channel doctor 2>&1 || true)
END_TIME=$(date +%s)
